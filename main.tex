...
On a data set of 80 million 64-bit keys, \MlpIndex only takes around 2 DRAM roundtrip time to perform 
...
struct Node {
    ...
    Key fullKey;
    // The hash table for this DS, shared by all nodes in this DS
    HashTable* hashTable;
...
structures we are aware of -- the Height Optimized Trie (HOT) \cite{hot_sigmod18} and the Adaptive Radix Tree (ART) \cite{arttrie_icde13}.
...
with its practical performance. Our implementation of \MlpIndex is available at \url{https://github.com/sillycross/mlpds}.
...
benchmarks multiple components of trie-based data structures and does not over-emphasize a certain component. 

Due to time limitation, we only tested two such distributions. 
In distribution $A$, the first 6 bytes have a small candidate set of a few elements for each byte, 
...
while the last 6 bytes have a small candidate set of a few elements for each byte.
...
with only a small probability). The final positive rate is controlled to be around 80\%. 
\lowerbound queries are sampled from the same distribution as the original data set is generated. 

Our benchmark rivals are Height Optimized Trie (HOT) \cite{hot_sigmod18} 
and Adaptive Radix Tree (ART) \cite{arttrie_icde13}, two fastest state-of-the-art 
ordered index structures to the best of our knowledge.
We also benchmark our performance against hash table, which cannot support \lowerbound queries. 
We included two hash table implementations, specifically, google's dense\_hash\_map \cite{densehashmap}, 
and the cuckoo hash that is used in \MlpIndex.
...
Recall that each cuckoo hash lookup simply looks like this:
...
they will take this advantage thanks to the tight and logic-less benchmark loop, and give out deceptive results. 
The solution is to use the same trick as in Section \ref{understandmlp}. 
We will hide the query text from compiler optimization and CPU out-of-order and speculative execution by 
...
We note that this ``query encryption'' only make significant difference on hash table, mainly due to its simple logic. 
For ordered index, all benchmarked implementation, including \MlpIndex, becomes slower, but only by a small degree. 
This is as expected: since CPU speculative execution only have a small window, it is not surprising that it 
