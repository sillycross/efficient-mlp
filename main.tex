...
Our superior speed 
comes from a combination of theoretical algorithm re-designs to leverage memory level parallelism, 
and pratical optimizations including a specially-modified hash table scheme, 
and using bitmaps to increases the fanout of the trie tree to 256.
...
depending on the lookup results. \insertion only writes to cache-resident addresses after the lookup 
...
This is elaborated in the ``Lowest Level Bitmap'' idea in paragraph \ref{lowestlevelbitmap}.
...
int QueryLCP(Tree* t, Key x) {
    ...
    // termination case (subtree with only 1 element):
    // This can only happen on the root of its parent tree,
