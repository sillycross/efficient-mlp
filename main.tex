...
While we could improve our \lookup operation to $O(1)$ by simply building a stand-alone hash table to hold all keys, 
we deem it as unnecessary since in practice our native $O(\log_D\log M)$ \lookup is 
already almost as fast as a carefully-engineered hash table could do.

The core idea of \MlpIndex is from x-fast trie \cite{xfast}, a data structure mostly considered in theory.
X-fast trie supports $O(\log M)$ \insertion, $O(1)$ \lookup, $O(\log\log M)$ \lowerbound, while using $O(n\log M)$ memory.
While its \lookup and \lowerbound performance is theoretically optimal as proven in \cite{ajtai88comb-lowerbound},
...
so similarily we need to materialize the ``2'' and ``3'' node before our newly inserted node.
...
as shown in Figure \ref{mlpindex_node}. It turns out that we should use the ``prefix'' as a node's index 
...
until we locate that node in the hash table. 
...
where n is the LCP node, as shown below.
